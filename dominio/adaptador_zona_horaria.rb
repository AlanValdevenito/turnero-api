class AdaptadorZonaHoraria
  def initialize(proveedor_fecha)
  ...
  def parsear_a_utc(fecha, hora)
    ...
    utc_time = @proveedor_fecha.construir_hora_desde_local(anio, mes, dia, hora_num, minuto)
  ...
  def adaptar_zona_horaria(turno)
    fecha_hora_local = @proveedor_fecha.cambiar_a_huso_horario_local(turno.fecha_hora)
  ...
  def adaptar_zona_horarios(horarios_utc)
    Array(horarios_utc).map { |dt| @proveedor_fecha.cambiar_a_huso_horario_local(dt) }
  end
end
