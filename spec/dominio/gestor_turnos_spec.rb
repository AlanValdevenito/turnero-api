...
describe GestorTurnos do
  ...
  describe '#disponibilidad_de_medico' do
  ...
  describe '#crear_turno' do
    ...
    context 'when ya existe un turno en la misma fecha y hora' do
      let(:usuario) { Usuario.new('otro@mail.com') }
    ...
    context 'when se alcanza el límite de turnos por especialidad' do
      let(:especialidad) { Especialidad.new('Dermatologia', 20, 2) }
      let(:medico) { Medico.new('Ana', 'Torres', 'DER123', especialidad) }
      let(:usuario) { Usuario.new('paciente@test.com') }
      let(:turnos_existentes) do
        [
          Turno.new(medico, usuario, hora_utc(2025, 7, 10, 10)),
          Turno.new(medico, usuario, hora_utc(2025, 7, 11, 10))
        ]
      end

      before(:each) do
        allow(contexto[:repositorio_turnos]).to receive(:buscar_por_usuario).with(usuario).and_return(turnos_existentes)
        allow(contexto[:repositorio_turnos]).to receive(:buscar_por_medico).with(medico).and_return([])
      end
  ...
  describe '#proximos_turnos_paciente' do
    ...
    context 'when no hay próximos turnos' do
      ...
      it 'tira error si no hay próximos turnos' do
        expect do
  ...
  describe '#modificar_estado_turno' do
    let(:turnos) do
      {
  ...
  describe '#historial_turnos_paciente' do
    ...
    it 'tira error si el paciente nunca reservó turnos' do
  ...
  describe '#cancelar_turno' do
    ...
    describe '#ocurre_proximas_24hs?' do
  ...
  describe '#cumple_limite_turnos?' do
    let(:datos_test) do
      {
        especialidad: Especialidad.new('Cardiologia', 30, 2),
        especialidad_sin_limite: Especialidad.new('Oftalmologia', 30, nil),
        usuario: Usuario.new('paciente@test.com')
      }
    end

    def crear_medico_cardiologia(especialidad)
      Medico.new('Juan', 'Perez', 'CAR123', especialidad)
    end

    context 'when no tiene turnos' do
      before(:each) do
        allow(contexto[:repositorio_turnos]).to receive(:buscar_por_usuario).with(datos_test[:usuario]).and_return([])
      end

      it 'devuelve true si el usuario no tiene turnos' do
        expect(contexto[:gestor_turnos].cumple_limite_turnos?(datos_test[:usuario], datos_test[:especialidad])).to eq(true)
      end
    end

    context 'when se supera el limite de turnos' do
      let(:test_data) do
        especialidad = Especialidad.new('Cardiologia', 30, 2)
        {
          especialidad:,
          medico: Medico.new('Juan', 'Perez', 'CAR123', especialidad),
          usuario: Usuario.new('paciente@test.com')
        }
      end
      ...
      let(:turnos) do
        [
          Turno.new(test_data[:medico], test_data[:usuario], hora_utc(2025, 7, 10, 10)),
          Turno.new(test_data[:medico], test_data[:usuario], hora_utc(2025, 7, 11, 10)),
          Turno.new(test_data[:medico], test_data[:usuario], hora_utc(2025, 7, 12, 10))
        ]
      end

      before(:each) do
        allow(contexto[:repositorio_turnos]).to receive(:buscar_por_usuario)
          .with(test_data[:usuario]).and_return(turnos)
      end

      it 'devuelve false cuando se supera el limite de turnos para una especialidad' do
        expect(contexto[:gestor_turnos].cumple_limite_turnos?(
                 test_data[:usuario],
                 test_data[:especialidad]
               )).to eq(false)
      end
    end
  end
