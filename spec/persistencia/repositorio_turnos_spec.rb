...
describe RepositorioTurnos do
  ...
  it 'deberia buscar por el id del medico' do
    turno = crear_turno_para(medico, usuario, DateTime.parse('2025-06-10T11:30:00'))
    turno_encontrado = repositorios[:repo_turnos].buscar_por_medico(medico)
    expect(turno_encontrado.first.usuario.id).to eq(usuario.id)
    expect(turno_encontrado.first.medico.id).to eq(medico.id)
    expect(turno_encontrado.first.fecha_hora.to_time).to eq(turno.fecha_hora.to_time)
  end

  it 'deberia buscar por id del turno' do
    turno = crear_turno_para(medico, usuario, DateTime.parse('2025-06-10T11:30:00'))
    turno_encontrado = repositorios[:repo_turnos].buscar_por_id(turno.id)
  ...
  it 'elimina todos los turnos de un medico' do
    repositorios[:repo_turnos].save(crear_turno_para(medico, usuario, DateTime.parse('2025-06-10T11:30:00')))
    repositorios[:repo_turnos].save(crear_turno_para(medico, usuario, DateTime.parse('2025-06-11T11:30:00')))
    expect(repositorios[:repo_turnos].buscar_por_medico(medico).size).to eq(2)
